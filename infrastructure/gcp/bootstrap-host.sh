#!/usr/bin/env bash
# Run with sudo as the development user on an Ubuntu/Debian host.
# Installs the host-native development environment; it does not start Docker.
set -euo pipefail
...
# Optional for non-GCP hosts; enabled by default for this GCP wrapper.
